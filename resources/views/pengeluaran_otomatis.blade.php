...
<head>
  ...
  <title>Input Pengeluaran Otomatis — LabMoneyLens</title>
...
<body>
  <div class="app">
    ...
    <main class="main">
      <div class="page-wrapper">
        ...
        <!-- Back Button -->
        <a href="{{ route('pengeluaran.pilih') }}" style="display:inline-flex; align-items:center; gap:6px; color:#dc2626; text-decoration:none; font-weight:600; font-size:13px; margin-bottom:10px;">
        ...
        <div class="page-hero">
          <div class="page-hero-info">
            <h1>Input Pengeluaran Otomatis</h1>
            <p>Scan foto struk belanja dan parsing otomatis.<br>Pengeluaran tidak boleh melebihi saldo yang tersedia.</p>
          </div>
        ...
        <div class="form-card">
          <div class="form-card-header">
            ...
            <div>
              <h2>Form Pengeluaran Otomatis</h2>
          ...
          <div class="form-card-body">
            ...
            <div id="preview-container" style="display:none; margin-bottom:20px; background: linear-gradient(135deg, #ffffff 0%, #fff9f9 100%); border: 2px solid #fecaca; border-radius: 14px; padding: 16px; text-align: center;">
              <div style="font-size: 12px; font-weight: 700; color: #991b1b; margin-bottom: 12px;"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="15" height="15" style="display:inline-block;vertical-align:middle;margin-right:3px;"><circle cx="12" cy="12" r="10"/><path d="m9 12 2 2 4-4"/></svg> Foto Struk Tersimpan</div>
            ...
            <div id="upload-required-notice" style="display:none; background:#fee2e2; border:1.5px solid #fecaca; border-radius:10px; padding:10px 16px; margin-bottom:16px; font-size:12px; color:#991b1b; font-weight:600;">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="15" height="15" style="display:inline-block;vertical-align:middle;margin-right:3px;"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3"/><path d="M12 9v4M12 17h.01"/></svg> Foto struk harus diunggah terlebih dahulu sebelum form dapat diisi.
            </div>

            <form id="receipt_form" method="POST" action="{{ route('pengeluaran.store-otomatis') }}" enctype="multipart/form-data" class="input-form">
              @csrf
  ...
  <script>
    ...
    async function analyzeReceipt(file) {
      document.getElementById('upload-filename').innerHTML += ' <span style="font-size:12px;color:#0d9488;">(Menganalisis...)</span>';
      const receiptType = document.getElementById("receipt_type")?.value || "pengeluaran";
      ...
      try {
        const response = await fetch("{{ route('receipt.parse') }}", {
          method: "POST",
          headers: {
        ...
        if (!response.ok) throw new Error(result?.error || result?.message || `Status ${response.status}`);
    ...
    function fillParsedReceipt(data) {
      if (data.nominal) {
        const nominalField = document.getElementById("nominal_0") || document.getElementById("nominal_1");
        if (nominalField) nominalField.value = data.nominal;
      }
      if (data.kuantiti) {
        const qtyField = document.getElementById("kuantiti_0") || document.getElementById("kuantiti_1");
        if (qtyField) qtyField.value = data.kuantiti;
      }
      if (data.uraian) {
        const uraianField = document.getElementById("uraian_0") || document.getElementById("uraian_1");
        if (uraianField) uraianField.value = data.uraian;
      }
      if (data.tanggal) {
        const tanggalField = document.getElementById("tanggal");
        if (tanggalField) tanggalField.value = data.tanggal;
      }
      if (data.kategori) {
        const select = document.getElementById("kategori_pengeluaran") || document.getElementById("id_jenis_pengeluaran");
        if (select) {
          ...
          for (const option of select.options) {
            if (option.text.toLowerCase().includes(lower) || lower.includes(option.text.toLowerCase())) {
              ...
              if (typeof syncKategoriSync === 'function') syncKategoriSync();
              break;
            }
          }
        }
      ...
      if (typeof updateTotalManual === 'function') updateTotalManual();
    }
